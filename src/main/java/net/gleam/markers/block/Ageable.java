    ...
    default void tryDegrade(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextFloat() < g) {
            this.getDegradationResultState(state).ifPresent((statex) -> {
            	MarkerBlockEntity entity = (MarkerBlockEntity)world.getBlockEntity(pos);
            	GameProfile owner = entity.getMarkerOwner();
            	String name = entity.getCustomNametag();
            	DefaultedList<ItemStack> items = entity.getItems();
            	int xp = entity.getXp();
                world.setBlockState(pos, statex);
                MarkerBlockEntity newEntity = new MarkerBlockEntity(pos, statex);
                newEntity.setMarkerOwner(owner);
