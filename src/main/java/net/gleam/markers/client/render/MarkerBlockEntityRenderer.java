package net.gleam.markers.client.render;
...
public class MarkerBlockEntityRenderer implements BlockEntityRenderer<MarkerBlockEntity> {
    ...
    public MarkerBlockEntityRenderer(Context ctx) {
        super();
    ...
    public SkullBlockEntityModel getSkull(BlockState state) {
    	SkullBlockEntityModel skull = new SkullEntityModel(blockAge>=2 ? renderLayer.getModelPart(EntityModelLayers.SKELETON_SKULL) : renderLayer.getModelPart(EntityModelLayers.PLAYER_HEAD));
    	skull.setHeadRotation(1f, 2f, 2f);
    	return skull;
    }
    
    @Override
    public void render(MarkerBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
    	
    	BlockState state = blockEntity.getCachedState();
    	blockAge = ((MarkerBase)state.getBlock()).getWeathered();
        ...
        if(blockEntity.getMarkerOwner() != null) {
        	this.skull = getSkull(state);
        	//Make the yaw a configurable value
        	float yaw = Float.max(10f,blockAge * 12f);
        	if(blockAge>=2)SkullBlockEntityRenderer.renderSkull(null, yaw, 0f, matrices, vertexConsumers, light, skull, SkullBlockEntityRenderer.getRenderLayer(SkullBlock.Type.SKELETON,null)); else SkullBlockEntityRenderer.renderSkull(null, yaw, 0f, matrices, vertexConsumers, light, skull, SkullBlockEntityRenderer.getRenderLayer(SkullBlock.Type.PLAYER,blockEntity.getMarkerOwner()));
        }
        ...
        if(blockEntity.getMarkerOwner() != null) {
        	String text= "";
            if(blockEntity.getMarkerOwner() != null) {
                text = blockEntity.getMarkerOwner().getName();
            }
            ...
            this.textRenderer.draw(text, 0, 0, 0xFFFFFF, false, matrices.peek().getModel(), vertexConsumers, false, 0, light);
            matrices.pop();
        }
    }
}